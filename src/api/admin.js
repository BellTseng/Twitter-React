import axios from "axios";

const baseUrl = 'https://rocky-citadel-44413.herokuapp.com'
const testUrl = 'https://3ce8-118-150-219-108.jp.ngrok.io'

// const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..gh10qIVTAuj-MItd1gY2daS8nJOZjqyLeNlzO_Kl_90'

const axiosInstance = axios.create({
  baseURL: testUrl,
});

axiosInstance.interceptors.request.use((config) => {
  const token = localStorage.getItem('authToken');
  if (token) {
    config.headers['Authorization'] = `Bearer ${token}`
    config.headers['ngrok-skip-browser-warning'] = 'any'
  }
  return config;
}, (error) => {
  console.error('[]', error)
});


export const adminGetTweets = async () => {
  try {
    const res = await axiosInstance.get(`${testUrl}/api/admin/tweets`);

    return res.data
  }
  catch (error) {
    console.error('[Get Tweets failed]: ', error);
  }
};

export const adminDeleteTweet = async (id) => {
  try {
    const res = await axiosInstance.delete(`${testUrl}/api/admin/tweets/${id}`);

    return res.data
  }
  catch (error) {
    console.error('[Delete Tweet failed]: ', error);
  }
}

export const adminGetUsers = async () => {
  try {
    const res = await axiosInstance.get(`${testUrl}/api/admin/users`);

    return res.data
  }
  catch (error) {
    console.error('[Delete Tweet failed]: ', error);
  }
}